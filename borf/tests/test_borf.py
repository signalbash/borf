...
class TestFindNextStop(unittest.TestCase):
    ...
    def test_next_stop_from_within(self):
        # "check this finds the NEXT stop codon when given a start position greater than 0/1"
        orf = "AMEATY*METABALL*"
...
class TestFindMaxOrfIndex(unittest.TestCase):
    ...
    def test_find_max_orf_index(self):
        ...
        self.assertEqual(find_max_orf_index(start_locs = [0,100], end_locs = [1000, 200]), (0,1000))

    def test_find_max_orf_index_offby1(self):
        # test when second index is greater by one
        self.assertEqual(find_max_orf_index(start_locs = [0,100], end_locs = [999, 1100]), (100,1100))

    def test_find_max_orf_index_equal(self):
        # test that first instance of the max is returned
        self.assertEqual(find_max_orf_index(start_locs = [0,100], end_locs = [1000, 1100]), (0,1000))
...
class TestFindLongestORF(unittest.TestCase):
    ...
    def test_find_longest_orf_output_format(self):
        # tests that a length 5 tupple output, and each is the correct numpy array type
    ...
    def test_find_longest_orf_stopsites(self):
        # check that the stop site is calculated as the * for seqs with it, and the last AA for those without
...
class TestReplaceLastStop(unittest.TestCase):
    ...
    def test_replace_last_stop(self):
        # check that the last * is trimmed from the orf sequence
        self.assertEqual(replace_last_stop('MEATBALL'), replace_last_stop('MEATBALL*'))
...
class TestAddUpstreamAAs(unittest.TestCase):
    ...
    def test_add_upstream_aa_output(self):
        ...
        orf_sequence, start_sites, stop_sites, orf_length, last_aa_is_stop = find_longest_orfs(aa_sequence)
        output = add_upstream_aas(aa_sequence, stop_sites, start_sites, orf_sequence, orf_length, min_upstream_length=5)
    ...
    def test_add_upstream_aa(self):
        ...
        aa_sequence = np.array(['ALONGERUPSTREAMMEATBALL'])
        orf_sequence, start_sites, stop_sites, orf_length, last_aa_is_stop = find_longest_orfs(aa_sequence) # this should already be tested and produce MMEATBALL
        output = add_upstream_aas(aa_sequence, stop_sites, start_sites, orf_sequence, orf_length, min_upstream_length=5)
    ...
    def test_add_upstream_aa_multi(self):
        ...
        aa_sequence = np.array(['ALONGERUPSTREAMMEATBALL', 'TWODOZENMEATBALLS', 'BROWNBEARMAULSGIANTSQUID'])
        orf_sequence, start_sites, stop_sites, orf_length, last_aa_is_stop = find_longest_orfs(aa_sequence) # this should already be tested and produce MMEATBALL
        output = add_upstream_aas(aa_sequence, stop_sites, start_sites, orf_sequence, orf_length, min_upstream_length=5)

        self.assertTrue(np.all(output[0] == np.array(['ALONGERUPSTREAMMEATBALL', 'TWODOZENMEATBALLS', 'BROWNBEARMAULSGIANTSQUID'])))
    ...
    def test_add_upstream_aa_exactupstream(self):
        ...
        aa_sequence = np.array(['BEARMEATBALLS'])
        orf_sequence, start_sites, stop_sites, orf_length, last_aa_is_stop = find_longest_orfs(aa_sequence) # this should already be tested and produce MMEATBALL
        output = add_upstream_aas(aa_sequence, stop_sites, start_sites, orf_sequence, orf_length, min_upstream_length=4)
...
class TestFilterObjects(unittest.TestCase):
    ...
    def test_filter_objects(self):
        ...
        self.assertTrue(np.all(output[0] == np.array(['A', 'B', 'I', 'J'])) and np.all(output[1] == np.array([1, 2, 2, 1])))
...
class TestTranslateAllFrames(unittest.TestCase):
    ...
    def test_translate_bothstrands(self):
        ...
        trans_correct = np.all(aa_frames == np.array(['MANATEE*', 'LFFGRVRH', 'WRTRPKN', 'YSSVAFA', 'GERDRRI', 'ILRSRSP']))
...
class TestConvertAANT(unittest.TestCase):
    ...
    def test_convert_nt_output_format(self):
        ...
        output = convert_start_stop_to_nt(start_sites, stop_sites, seq_length_nt, orf_length, frame, last_aa_is_stop)
...
class TestCheckFirstAA(unittest.TestCase):
    ...
    def test_check_first_aa_multi(self):
        ...
        self.assertTrue(np.all(check_first_aa(aa_sequence) == np.array(['M','ALT'])))
...
class TestCheckUniqueN(unittest.TestCase):
    ...
    def test_check_unique_n(self):
        # tests that a length 3 tupple output, and each is the correct numpy array type
        values = np.array(['MEATBALL', 'MEATBALL','BEAR','MEATBALL','MEATBALLS'])
...
class TestFindAllORFs(unittest.TestCase):
    ...
    def test_find_two_orfs(self):
        ...
        orf_correct = np.all(orf_sequence == np.array(['MEATBALL', 'MEATBALLBEAR']))
        ...
        last_aa_is_stop_correct = np.all(last_aa_is_stop == np.array([True, True]))
        matched_index_correct = np.all(matched_index == np.array([0, 0]))

        self.assertTrue(orf_correct and start_correct and stop_correct and orf_length_correct and last_aa_is_stop_correct and last_aa_is_stop_correct and matched_index_correct)
...
class TestAddOrfClass(unittest.TestCase):
    ...
    def test_add_orf_classification_complete(self):
        ...
        self.assertTrue(np.all(add_orf_classification(orf_df) == np.array(['complete'])))
    ...
    def test_add_orf_classification_incomplete_5prime(self):
        ...
        self.assertTrue(np.all(add_orf_classification(orf_df) == np.array(['incomplete_5prime'])))
    ...
    def test_add_orf_classification_incomplete_3prime(self):
        ...
        self.assertTrue(np.all(add_orf_classification(orf_df) == np.array(['incomplete_3prime'])))
    ...
    def test_add_orf_classification_incomplete(self):
        ...
        self.assertTrue(np.all(add_orf_classification(orf_df) == np.array(['incomplete'])))
    ...
    def test_add_orf_classification_multi(self):
        ...
        self.assertTrue(np.all(add_orf_classification(orf_df) == np.array(['complete', 'incomplete_5prime', 'incomplete_3prime', 'incomplete'])))
